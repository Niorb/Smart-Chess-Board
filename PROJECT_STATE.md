...
- [x] Fixed Match Start Lifecycle & Race Condition in Game Stream Cancellation (`Raspberry/app/lichess_engine.py`):
  - **Root Cause**: When a new game was launched (e.g. via seek or Stockfish AI challenge), cancelling the previous game stream task triggered its asynchronous `finally:` block. The outgoing stream unconditionally set `state_manager.game_status = "IDLE"` and recorded last game moves over the new match, causing the board to revert from `PLAYING` to `IDLE` immediately after displaying the player's color and activating idle gesture starter pawns.
  - **Active Stream Matching & Status Protection**: Added active game ID checks (`if self.current_game_id == game_id:`) to the `finally:` block and guarded seek cancellation so previous tasks never mutate `game_status` or override `last_game_moves` for newer matches. Explicitly enforced `state_manager.game_status = "PLAYING"` upon receiving `gameFull` events.
