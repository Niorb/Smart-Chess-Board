...
## Execution Environment & Remote Test Rules
> [!IMPORTANT]
> - **STRICT RULE**: NEVER run `pytest` or `npm` on the local machine.
> - ALL `pytest` test suites and verification commands MUST ONLY be executed remotely on the physical **Raspberry Pi** over SSH.
> - SSH Host: `ssh pi@pi`
> - Environment: `source ~/venv/chess/bin/activate`
...
## Handoff Protocol
- Review all code generated by **Core Game & State Engine**, **Chess AI & Lichess**, **Embedded & Hardware**, **Lighting & Animation**, and **Web Frontend** specialists.
- Run the full pytest test suite remotely on the Raspberry Pi before declaring any feature or bug fix complete.
