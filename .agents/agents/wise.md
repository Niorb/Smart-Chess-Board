...
- **Castling Invariant & Piece Type Validation**: Never infer castling solely from geometric coordinate transitions (e.g. `(4,0) -> (2,0)` or `(4,0) -> (6,0)`). Moving a Rook (e.g. `Re1-c1` or `Re1-g1`) or Queen 2 squares horizontally shares the King's castling file shift. In both physical tracking (`process_physical_state`, `sync_game`) and LED trace rendering (`render_trace`, `replay_learn`), always verify that the moving piece is a `chess.KING` and that `board.is_castling()` evaluates to `True` before arming `pending_castling_rook` or invoking `render_castle_trace`.
