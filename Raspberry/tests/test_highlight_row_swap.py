...
def test_highlight_with_row_swap():
    ...
    assert is_row_swapped(4) is False  # File e
    assert is_row_swapped(7) is False  # File h
    ...
    # For e1 (rank=0, file=4): row swap NOT active, mapped to physical rank 0
    leds_e1 = get_led_indices(0, 4)
    assert leds_e1 == get_led_indices(0, 4, swap_rows=False)
